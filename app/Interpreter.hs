...
interpret_block symbolTable (EXP_SRCBLOCK (cur:[])) = let (symbolTable2, val) = interpret_statement symbolTable cur in (symbolTable2, val)
...
interpret_statement symbolTable (EXP_SRCBLOCK x) = let (symtab, val) = interpret_block symbolTable $ EXP_SRCBLOCK x in (Map.intersection symtab symbolTable, val)
interpret_statement symbolTable (EXP_ASSIGNMENT (EXP_LHS tokens) exp) = (symbolTable2, exp_interp)
  where
    ...
    symbolTable2 = Trace.traceShow (tokens, exp_interp) $ Map.insert (EXP_LHS tokens) (typeID, val_or_func, exp_interp, exp) symbolTable
interpret_statement symbolTable (EXP_IF cond expr_true expr_false) =
  ...
  where
    ...
    (expr_true_symboltable, expr_true_val)   = interpret_statement cond_symboltable expr_true
    (expr_false_symboltable, expr_false_val) = interpret_statement cond_symboltable expr_false
...
interpret_statement symbolTable (EXP_DIV exp1 exp2) =
  ...
  where
    (symboltable1, interp1) = interpret_statement symbolTable exp1
    (symboltable2, interp2) = interpret_statement symbolTable exp2
interpret_statement symbolTable (EXP_MOD exp1 exp2) =
  case interp1 of
    VAL_NUM x ->
      case interp2 of
      VAL_NUM y -> (Map.union symboltable2 symboltable1, VAL_NUM $ x `mod` y)
      _ -> error "mismatched types"
    _ -> error "mismatched types"
  where
    (symboltable1, interp1) = interpret_statement symbolTable exp1
...
interpret_statement symbolTable (EXP_VALUE value) =
  case value of
    ...
    TOK_USERDEF x ->
      let
        lookup = Map.lookup (EXP_LHS $ [TOK_USERDEF x]) symbolTable
      in
        case lookup of
